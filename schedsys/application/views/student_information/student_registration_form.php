...
$person2=$this->mainmodel->student_data($pid);
...
$y=date("Y");
$yoe=($mode=='add')?date("Y"):$person2[0]['yoe'];
$code=($mode=='add')?$this->mainmodel->numformat(6,$this->mainmodel->getMaxVal("sys_s_student","count","where yoe='$y'")+1):$this->mainmodel->numformat(6,($person2[0]['count']));

$student_ID=($mode=='add')?$yoe."-".$code:$person2[0]['student_id'];
?>
...
<div class='col-sm-4'><input type='text' class='form-control' id='bplace' value="<?= $bplace; ?>"></div>
</div>
...
<div class='col-sm-4'><input type='text' class='form-control' id='student_id' value='<?= $student_ID; ?>'></div>
...
<div class='col-sm-4'></div>
<div class='col-sm-2'><span></span></div>
<div class='col-sm-4'><button class='btn btn-primary form-control' onclick='StudentvalidateSave()'>Save</button></div>
</div>
...
$.post(URL+"index.php/main/loadStudentSave",{student_id:student_id,status:status,pid:pid,fname:fname,mname:mname,lname:lname,ename:ename,gender:gender,bdate:bdate,bplace:bplace,email:email,contact:contact,mode:mode}).done(function(data){

n=JSON.parse(data);
if(n.msg==""){ setStudentTable(); $('#dialog1').jseDialog('close');  bootbox.alert("Done Saving", function() {
